...
describe("SqlTableCheckpointStore", () => {
  ...
  it("generates postgres-shaped SQL", async () => {
    ...
    expect(mem.lastQuerySql).toBe(
      `SELECT "payload" FROM "s3flow_checkpoint" WHERE "job_id" = $1`,
    );
    await store.save("j1", sample());
    expect(mem.lastExecuteSql).toMatch(
      /^INSERT INTO "s3flow_checkpoint" \("job_id", "payload"\) VALUES \(\$1, \$2\) ON CONFLICT \("job_id"\) DO UPDATE SET "payload" = EXCLUDED\."payload"$/,
    );
  });
