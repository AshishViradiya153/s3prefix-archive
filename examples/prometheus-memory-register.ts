 ...
 *
 * Install: `prom-client` is already a dependency of `s3flow` (metrics); this example prints text exposition.
 */
import { createWriteStream } from "node:fs";
import { Registry } from "prom-client";
import { MemoryStorageProvider, pumpArchiveToWritable } from "s3flow";
...
async function main(): Promise<void> {
  ...
  await pumpArchiveToWritable(createWriteStream("./_s3flow-prom-example.zip"), {
