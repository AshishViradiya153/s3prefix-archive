...
Runnable sketches aligned with published imports (`s3flow`, `s3flow/platform`, …). From a project that depends on `s3flow`, copy a file and adjust environment variables and ARNs/buckets. Deeper guides live in **[`docs/README.md`](../docs/README.md)**.
...
CLI (`npx s3flow archive`, `npx s3flow index`, `npx s3flow benchmark`) is documented in the root [README.md](../README.md).
