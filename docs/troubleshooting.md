...
For large exports, prefer **server-side** `s3prefix-archive`; for small sets, **presigned GET** URLs and a client-side zip library may suffice. Use **`recommendArchiveExecutionSurface`** for a stable hint. See [presigned-urls.md](presigned-urls.md).
